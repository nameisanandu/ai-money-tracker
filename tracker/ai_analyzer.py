...
def detect_expense_anomalies(transactions, multiplier=2.0):
    ...
    for _, row in expense_df.iterrows():
        if row['amount'] >= multiplier * avg_expense:
            insights.append(
                f"Unusual expense detected: ₹{row['amount']:.2f} on {row['category']} "
                f"({row['amount'] / avg_expense:.1f}x your average). "
                f"Description: {row['description'] or 'N/A'}"
            )

    return insights[:3]  # Limit anomalies shown
...
def get_personalized_suggestions(transactions):
    ...
    # Suggest budget for high-spend categories
    high_spend_categories = ['Shopping', 'Food', 'Travel', 'Entertainment']
...
def get_financial_insights(transactions, loans=None):
    ...
    # A. Spending concentration detection (categories > 40%)
    expense_insights = analyze_expense_patterns(transactions, threshold=0.4)
    insights.extend(expense_insights)

    # B. Expense anomaly detection (2x average)
    anomaly_insights = detect_expense_anomalies(transactions, multiplier=2.0)
    insights.extend(anomaly_insights)

    # C. Personalized financial suggestions
    suggestions = get_personalized_suggestions(transactions)
    insights.extend(suggestions)

    # Total summary insight
    ...
    if not expense_df.empty and 'date' in expense_df.columns:
        ...
        if len(this_week) > 0:
            ...
            if monthly_avg > 0 and week_total > monthly_avg * 0.5:
                insights.append(
                    f"You spent unusually high this week (₹{week_total:.0f}) compared to your "
                    "monthly average. Review recent transactions."
                )

    # Debt-related insights if loans are provided
    if loans:
        total_monthly_emi = sum(getattr(loan, 'emi_amount', 0) for loan in loans)
        if total_monthly_emi and total_income > 0:
            avg_monthly_income = total_income / max(
                1, len(income_df['date'].dt.to_period('M').unique())
            )
            if avg_monthly_income > 0:
                emi_pct = (total_monthly_emi / avg_monthly_income) * 100
                if emi_pct >= 40:
                    insights.append(
                        f"Your monthly EMIs (₹{total_monthly_emi:.0f}) are {emi_pct:.1f}% of your average monthly income. "
        ...
        for loan in loans:
            ...
            if rem_emis > 0:
                insights.append(
                    ...
                    f"Approximate remaining balance is ₹{loan.remaining_balance():.0f}."
                )

    return insights[:10]  # Limit for cleaner display
